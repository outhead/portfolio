    ...
    <>
      ...
      {heroes.length > 1 ? (
        <div className="flex items-center gap-2 mt-4 md:mt-5">
          ...
          <span className="ml-3 font-mono text-[16px] tracking-[0.14em] uppercase text-white/45 tabular-nums">
      ...
      {zoomed ? (
        ...
        >
          ...
          {heroes.length > 1 ? (
            <div className="fixed top-[max(1.5rem,env(safe-area-inset-top))] left-5 z-20 font-mono text-[16px] tracking-[0.14em] uppercase text-white/55 tabular-nums pointer-events-none">
              {String(active + 1).padStart(2, "0")} /{" "}
              {String(heroes.length).padStart(2, "0")}
            </div>
          ) : null}
