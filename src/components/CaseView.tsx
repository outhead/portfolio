...
import LedShowcase from "@/components/LedShowcase";
  ...
  return (
    <>
      ...
      >
        ...
        <Link
          href={localizedHref("/#portfolio", locale)}
        ...
        >
          ...
          <span className="sr-only">{t.allProjects}</span>
          <LedText text={t.allProjects} className="h-[9px] w-auto" />
        </Link>
        ...
        <div className="max-w-4xl flex-1 min-w-0">
              ...
              <div key={i} id={`section-${i + 1}`} className="mb-16 md:mb-24 scroll-mt-24">
                ...
                {section.heroes && section.heroes.length > 0 && (
                  <div className="mb-8 md:mb-10">
                    <HeroSlider
                      ...
                      label={section.heroes.length > 1 ? t.postersLabel(section.heroes.length) : undefined}
                    />
                  </div>
                )}
                ...
                {hasStructured ? (
                  <div className="space-y-6 max-w-3xl">
                    {section.context && (
                      <div>
                        <CaseLabel>{t.task}</CaseLabel>
                        {renderProse(section.context)}
                      </div>
                    )}
                    {section.approach && (
                      <div>
                        <CaseLabel>{t.approach}</CaseLabel>
                ...
                {section.showcase && <LedShowcase className="mt-8 md:mt-10" />}
                      ...
                      <ol className="relative flex gap-0 min-w-max">
                        ...
                        {section.timeline.map((tl, idx) => (
                          <li key={idx} className="relative flex flex-col gap-2 pr-8 w-[220px] md:w-[240px] shrink-0">
                            ...
                            {tl.note && (
                              <div className="text-[16px] text-white/50 leading-snug">{tl.note}</div>
                            )}
                          </li>
                        ))}
                      </ol>
                  ...
                  <div className={`mt-10 md:mt-12 ${section.videoBlock.narrow ? "max-w-2xl mx-auto" : ""}`}>
                    <div className="overflow-hidden rounded-2xl border border-white/[0.06] bg-black">
                      ...
                      <video
                        ...
                        aria-label={section.videoBlock.alt ?? `${section.title} — video`}
                      />
                    </div>
                    {section.videoBlock.cta && project.tryUrl && (
                      <div className="mt-5 flex flex-col sm:flex-row sm:items-center gap-4">
                        ...
                        >
                          ...
                          <span className="sr-only">{t.tryIt}</span>
                          <LedText text={t.tryIt} className="h-[12px] w-auto" />
                        </a>
          ...
          {project.screenshots && project.screenshots.length > 0 &&
            ...
            <div className="mt-16 mb-12">
              <CaseLabel className="mb-6" tone="text-white/35">{t.screenshots}</CaseLabel>
              <ImageLightbox
          ...
          {project.links && project.links.length > 0 && (() => {
            const groups = project.links.reduce<Record<string, NonNullable<typeof project.links>>>((acc, link) => {
              const cat = link.category || t.proofs;
              if (!acc[cat]) acc[cat] = [];
              acc[cat].push(link);
              return acc;
            }, {});
            const order = [
              ...
              "Кросс-продуктовая интеграция", "Cross-product integration",
              "MTS Voice Ecosystem",
              "Ссылки", "Links",
            ];
            ...
            return (
              <div className="mt-16 mb-8">
                <CaseLabel className="mb-6" tone="text-white/35">{t.proofs}</CaseLabel>
        ...
        {project.sections && project.sections.length > 1 && (
          <aside className="hidden lg:block w-44 xl:w-48 flex-shrink-0">
            <div className="sticky top-24">
              <CaseLabel className="mb-3" tone="text-white/35">{t.contents}</CaseLabel>
      ...
      <section className="relative z-[1] px-5 md:px-[6%] lg:px-[10%] xl:px-[14%] 2xl:px-[max(14%,calc((100%_-_1680px)/2))] py-12 md:py-16 bg-black border-t border-white/[0.06]">
        <div className="max-w-3xl">
          <CaseLabel className="mb-4" tone="text-white/35">{t.openToOffers}</CaseLabel>
      ...
      <section className="relative z-[1] px-5 md:px-[6%] lg:px-[10%] xl:px-[14%] 2xl:px-[max(14%,calc((100%_-_1680px)/2))] bg-black border-t border-white/[0.06]">
        <div className="grid grid-cols-2 gap-px">
          {prev ? (
            <Link
              href={localizedHref(`/cases/${prev.slug}`, locale)}
            ...
            >
              <div className="text-white/30 mb-2 flex items-center gap-2">
                ...
                <LedText text={t.prev} className="h-[9px] w-auto" />
              </div>
            ...
            >
              <div className="text-white/30 mb-2 flex items-center gap-2">
                ...
                <LedText text={t.back} className="h-[9px] w-auto" />
              </div>
              <div className="text-sm md:text-base text-white/50 group-hover:text-white/80 transition-colors">
                {t.allProjects}
              </div>
          ...
          {next ? (
            <Link
              href={localizedHref(`/cases/${next.slug}`, locale)}
            ...
            >
              <div className="text-white/30 mb-2 flex items-center gap-2 justify-end">
                ...
                <LedText text={t.next} className="h-[9px] w-auto" />
                <LedText text="→" className="h-[10px] w-auto" />
              </div>
              <div className="text-sm md:text-base text-white/50 group-hover:text-white/80 transition-colors">
                {next.title}
