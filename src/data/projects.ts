...
export const projects: Project[] = [
  ...
  },
  {
    ...
    year: 2022,
    description:
      "Развернул дизайн-функцию из 80 человек и разморозил ЕСО — флагманский внутренний портал на 50 000 пользователей, который стоял в макетах два года. CX Awards 2024. Развивал open-source дизайн-систему Consta — её используют 180+ цифровых продуктов компании.",
    longDescription:
    ...
    sections: [
      ...
      },
      {
        ...
        result:
          "Раскатан на 60+ организаций ГПН: 50 000+ пользователей, 550+ сервисов в каталоге. Оценочный экономический эффект — сотни миллионов ₽ в год (по средней ставке часа × частоте операций × числу пользователей). Победитель CX Awards 2024 в номинации «Работа с персоналом как с внутренним клиентом» — раскатан и награждён уже после моего ухода. Переговоры и проектирование — на мне; релиз и поддержка — на команде.",
        callouts: [
      ...
      },
      {
        ...
        approach:
          "Команда из 4 человек: я как CPO — бэклог приоритетов, бюджет и адвокация внутри/снаружи компании. Алексей Титяев (арт-директор в прямом подчинении) — компонентная часть и связь с продуктовыми командами. Full-time-разработчик — релизы, инфраструктура, поддержка. Дизайнер — документация и Figma-библиотека.",
        result:
      ...
      },
      {
        ...
        context:
          "Consta — production-DS, на ней работают десятки внутренних продуктов ГПН. Здесь — два показательных. ЭРА Бурения — продукт моей команды, лучшие дизайнеры и Алексей Титяев. Юпитер — продукт другого отдела, выбравший Consta как стандарт. Разные домены, разные команды — один язык интерфейса.",
        result:
          "**ЭРА Бурения — мой продукт.** Экономическая Расчётная Аналитическая модель ствола скважины. Один продукт — три радикально разных контекста использования: промысел (полевые условия), НТЦ, центр управления. Адаптивность под все экраны — последняя задача Алексея Титяева перед моим уходом.\n\n**Юпитер — пример распространения Consta вне моей команды.** Корпоративный поиск и мониторинг IT-инфраструктуры ГПН: поиск по объектам компании (BLPS, БРД, ДРП, RCL, Linux), лента событий, инфраструктурные карты SAP-блоков. Команда выбрала Consta как стандарт — без моего участия в продуктовых решениях.",
        callouts: [
      ...
      },
      {
        ...
        callouts: [
          ...
          { value: "5+", label: "моделей в дизайн-процессе" },
