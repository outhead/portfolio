...
const PRESETS: { color: string; logo: string; label: string }[] = [
  { color: "#FF2436", logo: "/images/logos/mts.svg", label: "МТС" },
  { color: "#1C92E5", logo: "/images/logos/gazpromneft.svg", label: "ГПН" },
  { color: "#2E6BFF", logo: "/images/logos/ozon.svg", label: "OZON" },
  { color: "#3DDC84", logo: "/images/logos/smiley.svg", label: ":)" },
];

const MODES: { v: CursorMode; label: string; hint: string }[] = [
  { v: "stir", label: "Stir", hint: "тащит кубы за курсором" },
  { v: "push", label: "Push", hint: "расталкивает от курсора" },
  { v: "vortex", label: "Vortex", hint: "закручивает воронкой" },
  { v: "off", label: "Off", hint: "курсор не влияет" },
];
    ...
    <main className="min-h-screen bg-[#080807] text-white px-4 py-8 md:px-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-baseline justify-between mb-6">
          ...
          <span className="text-[11px] text-white/40 tabular-nums">активных кубов: {count}</span>
        </div>
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-6">
          ...
          <div>
            <div className="relative aspect-[16/10] rounded-2xl overflow-hidden bg-[#0b0b0a] border border-white/[0.06]">
              ...
              <div className="absolute bottom-3 left-0 right-0 text-center text-[11px] text-white/35 pointer-events-none">
                наведи и веди курсором по области
              </div>
            </div>
          ...
          <div className="space-y-5 rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4">
            <div>
              <div className="text-[11px] uppercase tracking-wider text-white/50 mb-2">Курсор</div>
              ...
              <p className="text-[11px] text-white/35 mt-1.5">{MODES.find((m) => m.v === ui.cursorMode)?.hint}</p>
            </div>

            <Slider label="Радиус курсора" value={ui.cursorRadius} min={0.4} max={2.5} step={0.05}
              onChange={(v) => set("cursorRadius", v)} fmt={(v) => v.toFixed(2)} />
            <Slider label="Сила курсора" value={ui.cursorPush} min={0} max={100} step={1}
              onChange={(v) => set("cursorPush", v)} />

            <hr className="border-white/10" />

            <Slider label="Гравитация" value={ui.gravity} min={5} max={60} step={1}
              onChange={(v) => set("gravity", v)} />
            <Slider label="Отскок (restitution)" value={ui.restitution} min={0} max={0.7} step={0.02}
              onChange={(v) => set("restitution", v)} fmt={(v) => v.toFixed(2)} />
            <Slider label="Трение" value={ui.friction} min={0} max={1} step={0.02}
              onChange={(v) => set("friction", v)} fmt={(v) => v.toFixed(2)} />
            <Slider label="Размер кубика" value={ui.cubeScale} min={0.35} max={1.1} step={0.02}
              onChange={(v) => set("cubeScale", v)} fmt={(v) => v.toFixed(2)} />

            <hr className="border-white/10" />

            <Slider label="Скорость спавна (куб/с)" value={ui.spawnRate} min={2} max={80} step={1}
              onChange={(v) => set("spawnRate", v)} />
            <Slider label="Макс. кубов" value={ui.maxCubes} min={5} max={90} step={1}
              onChange={(v) => set("maxCubes", v)} />

            <label className="flex items-center justify-between text-[12px] text-white/70">
              <span>Сыпать без курсора</span>
