...
// напрямую PostgREST по anon-ключу (видны только switches/joined/solved).
...
const FN = `${SB_URL}/functions/v1/pair`;
...
export async function pairState(id: string): Promise<PairState | null> {
  try {
    const res = await fetch(
      `${SB_URL}/rest/v1/pair_session?id=eq.${id}&select=switches,joined,solved`,
